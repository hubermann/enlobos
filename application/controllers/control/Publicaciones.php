<?php 

class publicaciones extends CI_Controller{


public function __construct(){

	parent::__construct();
	$this->load->model('publicacion');
	$this->load->model('categoria_publicacion');
	$this->load->helper('url');
	$this->load->library('session');

	//Si no hay session redirige a Login
	if(! $this->session->userdata('logged_in')){
	redirect('dashboard');
	}

	if( ! ini_get('date.timezone') ){
	    date_default_timezone_set('GMT');
	    setlocale(LC_ALL,"es_ES");
	    setlocale(LC_TIME, 'es_AR');
	}

	$this->data['thumbnail_sizes'] = array(); //thumbnails sizes 

}

public function index(){
	//Pagination
	$per_page = 15;
	$page = $this->uri->segment(3);
	if(!$page){ $start =0; $page =1; }else{ $start = ($page -1 ) * $per_page; }
		$data['pagination_links'] = "";
		$total_pages = ceil($this->publicacion->count_rows() / $per_page);

		if ($total_pages > 1){ 
			for ($i=1;$i<=$total_pages;$i++){ 
			if ($page == $i) 
				//si muestro el índice de la página actual, no coloco enlace 
				$data['pagination_links'] .=  '<li class="active"><a>'.$i.'</a></li>'; 
			else 
				//si el índice no corresponde con la página mostrada actualmente, coloco el enlace para ir a esa pagina 
				$data['pagination_links']  .= '<li><a href="'.base_url().'control/publicaciones/'.$i.'" > '. $i .'</a></li>'; 
		} 
	}
	//End Pagination

	$data['title'] = 'publicaciones';
	$data['menu'] = 'control/publicaciones/menu_publicacion';
	$data['content'] = 'control/publicaciones/all';
	$data['query'] = $this->publicacion->get_records($per_page,$start);

	$this->load->view('control/control_layout', $data);

}

//detail
public function detail(){

$data['title'] = 'publicacion';
$data['content'] = 'control/publicaciones/detail';
$data['menu'] = 'control/publicaciones/menu_publicacion';
$data['query'] = $this->publicacion->get_record($this->uri->segment(4));
$this->load->view('control/control_layout', $data);
}


//new
public function form_new(){
$this->load->helper('form');
$data['title'] = 'Nuevo publicacion';
$data['content'] = 'control/publicaciones/new_publicacion';
$data['menu'] = 'control/publicaciones/menu_publicacion';
$this->load->view('control/control_layout', $data);
}

//create
public function create(){

	$this->load->helper('form');
	$this->load->library('form_validation');
$this->form_validation->set_rules('titulo', 'Titulo', 'required');

$this->form_validation->set_rules('descripcion_corta', 'Descripcion_corta', 'required');

$this->form_validation->set_rules('descripcion_completa', 'Descripcion_completa', 'required');


$this->form_validation->set_rules('categoria_id', 'Categoria_id', 'required');

$this->form_validation->set_rules('provincia_id', 'Provincia_id', 'required');


	
	if ($this->form_validation->run() === FALSE){

		$this->load->helper('form');
		$data['title'] = 'Nuevo publicaciones';
		$data['content'] = 'control/publicaciones/new_publicacion';
		$data['menu'] = 'control/publicaciones/menu_publicacion';
		$this->load->view('control/control_layout', $data);

	}else{
		
		
			$this->load->helper('url');
			$slug = url_title($this->input->post('titulo'), 'dash', TRUE);
		

$newpublicacion = array( 'titulo' => $this->input->post('titulo'), 
 'descripcion_corta' => $this->input->post('descripcion_corta'), 
 'descripcion_completa' => $this->input->post('descripcion_completa'), 
 'owner' => $this->input->post('owner'), 
 'visitas' => $this->input->post('visitas'), 
 'tags' => $this->input->post('tags'), 
 'categoria_id' => $this->input->post('categoria_id'), 
 'provincia_id' => $this->input->post('provincia_id'), 
 'ciudad' => $this->input->post('ciudad'), 
 'modalidad_horarios' => $this->input->post('modalidad_horarios'), 
 'finalizado' => 0, 
 'aprobado' => 1, 
 'slug' => $slug, 
);
		#save
		$this->publicacion->add_record($newpublicacion);
		$this->session->set_flashdata('success', 'publicacion creado. <a href="publicaciones/detail/'.$this->db->insert_id().'">Ver</a>');
		redirect('control/publicaciones', 'refresh');

	}



}

//edit
public function editar(){
	$this->load->helper('form');
	$data['title']= 'Editar publicacion';	
	$data['content'] = 'control/publicaciones/edit_publicacion';
	$data['menu'] = 'control/publicaciones/menu_publicacion';
	$data['query'] = $this->publicacion->get_record($this->uri->segment(4));
	$this->load->view('control/control_layout', $data);
}

//update
public function update(){
	$this->load->helper('form');
	$this->load->library('form_validation'); 
$this->form_validation->set_rules('titulo', 'Titulo', 'required');

$this->form_validation->set_rules('descripcion_corta', 'Descripcion_corta', 'required');

$this->form_validation->set_rules('descripcion_completa', 'Descripcion_completa', 'required');




	$this->form_validation->set_message('required','El campo %s es requerido.');

	if ($this->form_validation->run() === FALSE){
		$this->load->helper('form');

		$data['title'] = 'Nuevo publicacion';
		$data['content'] = 'control/publicaciones/edit_publicacion';
		$data['menu'] = 'control/publicaciones/menu_publicacion';
		$data['query'] = $this->publicacion->get_record($this->input->post('id'));
		$this->load->view('control/control_layout', $data);
	}else{		
		$id=  $this->input->post('id');

	
			$this->load->helper('url');
			$slug = url_title($this->input->post('titulo'), 'dash', TRUE);
		

		$editedpublicacion = array(  
'titulo' => $this->input->post('titulo'),

'descripcion_corta' => $this->input->post('descripcion_corta'),

'descripcion_completa' => $this->input->post('descripcion_completa'),

'owner' => $this->input->post('owner'),

'visitas' => $this->input->post('visitas'),

'tags' => $this->input->post('tags'),

'categoria_id' => $this->input->post('categoria_id'),

'provincia_id' => $this->input->post('provincia_id'),

'ciudad' => $this->input->post('ciudad'),

'modalidad_horarios' => $this->input->post('modalidad_horarios'),

'finalizado' => $this->input->post('finalizado'),

'aprobado' => $this->input->post('aprobado'),

'slug' => $slug,
);
		#save
		$this->session->set_flashdata('success', 'publicacion Actualizado!');
		$this->publicacion->update_record($id, $editedpublicacion);
		if($this->input->post('id')!=""){
			redirect('control/publicaciones', 'refresh');
		}else{
			redirect('control/publicaciones', 'refresh');
		}



	}



}


//delete comfirm		
public function delete_comfirm(){
	$this->load->helper('form');
	$data['content'] = 'control/publicaciones/comfirm_delete';
	$data['title'] = 'Eliminar publicacion';
	$data['menu'] = 'control/publicaciones/menu_publicacion';
	$data['query'] = $data['query'] = $this->publicacion->get_record($this->uri->segment(4));
	$this->load->view('control/control_layout', $data);


}

//delete
public function delete(){

	$this->load->helper('form');
	$this->load->library('form_validation');

	$this->form_validation->set_rules('comfirm', 'comfirm', 'required');
	$this->form_validation->set_message('required','Por favor, confirme para eliminar.');


	if ($this->form_validation->run() === FALSE){
		#validation failed
		$this->load->helper('form');

		$data['content'] = 'control/publicaciones/comfirm_delete';
		$data['title'] = 'Eliminar publicacion';
		$data['menu'] = 'control/publicaciones/menu_publicacion';
		$data['query'] = $this->publicacion->get_record($this->input->post('id'));
		$this->load->view('control/control_layout', $data);
	}else{
		#validation passed
		$this->session->set_flashdata('success', 'publicacion eliminado!');

		$prod = $this->publicacion->get_record($this->input->post('id'));
			
		

		$this->publicacion->delete_record($this->input->post('id'));
		redirect('control/publicaciones', 'refresh');
		

	}
}


} //end class

?>