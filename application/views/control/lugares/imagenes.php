...
function confirma_eliminar(idvar, urldel) {
    ...
    if (result==true) {
        ...
        $.ajax({
            url: "<?php echo base_url('control/lugares/delete_imagen/');?>/"+idvar,
...
function update_main(idimagen,idpreview){
    ...
    xhr.open('POST', "<?php echo base_url('/control/lugares/main_imagen_update/') ?>", true);
    xhr.onload = function () {
        if (xhr.status === 200) {
...
<h4>Imagenes de la lugar</h4>
<hr>
<div class="panel panel-default">
    <div class="panel-body">
    <?php 
    $atts = array('id' => 'form_imagenes', 'class' => "navbar-form navbar-left", 'role'=> 'search');
    echo form_open_multipart(base_url('control/lugares/add_imagen'), $atts);
...
<?php 
    ...
    echo form_open(base_url('control/lugares/guardar_orden'), $atts);
    echo form_hidden('id', $this->uri->segment(4));
if($query_imagenes){
    ...
    foreach ($query_imagenes as $imagen) {
        $lugar = $this->lugar->get_record($this->uri->segment(4));
        $actual_principal = $lugar->main_image;
        ...
        <div class="container_img"><img src="'.base_url('images-lugares/'.$imagen->filename).'" width="120" alt="" /></div>
        <p onclick="confirma_eliminar('.$imagen->id.')" class="btn btn-default btn-xs" role="button">Quitar imagen</p>
        <input type="hidden" id="idlugar" name="idlugar" value="'.$this->uri->segment(4).'" />
   ...
   <div class="col-md-12"> <button type="submit" class="btn btn-success btn-xs">Guardar orden</button></div>
    ...
    echo '<p>Aun no tiene imagenes &eacute;sta lugar.</p>';
}
echo form_close();
?>
